...
export class WorksheetsRunner {
  ...
  constructor(private config: RunnerConfig) {
    this.auth = authService.login(config.userName, config.password).then(res => {
      if (!res) {
        throw new Error('Failed to login');
      }
      setBearerToken(authService.getCurrentToken());
      return res;
    }).catch(e => {
  ...
  async run(appPath: string, filePath: string, functionName?: string): Promise<any> {
    await this.auth;
  ...
  async getScheduledTasks(days?: string, time?: string) {
    ...
    return (await axios.get<AppLink>(url, {
  ...
  async startScheduledTask(taskId: string): Promise<number> {
    const url = `${DEFAULT_BASE_URL}/api/scheduled-tasks/start/${taskId}/agent1`;
    return (await axios.post<number>(url, null, {
      headers: authService.getAuthHeaders()
    })).data;
  }

  async endScheduledTask(taskStatusId: string, status: string, result: any): Promise<void> {
    const url = `${DEFAULT_BASE_URL}/api/scheduled-tasks/end`;
    await axios.put<unknown>(url, { taskStatusId, status, result }, {
      headers: authService.getAuthHeaders()
    });
  }
